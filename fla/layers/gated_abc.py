...
from fla.ops.gated_abc.chunk import gated_chunk_abc
...
class GatedABCAttention(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.s_proj = nn.Sequential(nn.Linear(hidden_size, gate_low_rank_dim, bias=False),
                                    nn.Linear(gate_low_rank_dim, self.num_heads * self.num_slots, bias=True))
        self.sg_proj = nn.Sequential(nn.Linear(hidden_size, gate_low_rank_dim, bias=False),
                                     nn.Linear(gate_low_rank_dim, self.num_heads * self.num_slots, bias=True))
    ...
    def forward(
        ...
        **kwargs
        ...
        s_g = rearrange(self.sg_proj(hidden_states), 'b t (h m) -> b h t m', h=self.num_heads)
        s_g = F.logsigmoid(s_g.float()) / self.gate_logit_normalizer

        last_state = past_key_values[self.layer_idx] if use_cache else None
        o, last_state = gated_chunk_abc(q, k, v, s, s_g, initial_state=last_state, output_final_state=use_cache)
