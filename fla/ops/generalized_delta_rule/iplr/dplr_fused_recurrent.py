...
def fused_recurrent_fwd_kernel(
    ...
    p_r = r + i_bh * s_qk_h + i_k * BK + tl.arange(0, BK)
...
def fused_recurrent_bwd_kernel(
    ...
    dr,  # gradient of receptance / query [NV, B, H, L, K]
    ...
    p_r = r + i_bh * s_qk_h + i_k * BK + tl.arange(0, BK) + (T - 1) * K
    p_w = w + i_bh * s_qk_h + i_k * BK + tl.arange(0, BK) + (T - 1) * K
    ...
    p_dq = dr + (i_bh + i_v * B * H) * s_qk_h + i_k * BK + tl.arange(0, BK)
    ...
    for i in range(0, T):
        ...
        tl.store(p_dq, d_r.to(p_dq.dtype.element_ty), mask=mask_bk)
...
class FusedRecurrentFunction(torch.autograd.Function):
    ...
    @staticmethod
    @contiguous
    def forward(ctx, r, w, k, v, alpha, beta, scale=None, initial_state=None, output_final_state=False):
        ...
        o = r.new_empty(NK, B, H, T, V)

        if output_final_state:
            final_state = r.new_empty(B, H, K, V, dtype=torch.float32)
        else:
            final_state = None
        ...
        fused_recurrent_fwd_kernel[grid](
            r, w, k, v, alpha, beta, o, ha, initial_state, final_state,
            r.stride(1),
            v.stride(1),
            scale,
            B=B, H=H, T=T, K=K, V=V,
        ...
        o = o.squeeze(0)
        ctx.save_for_backward(r, w, k, v, alpha, beta, ha, initial_state)
        ctx.scale = scale
    ...
    def backward(ctx, do, dht):
        r, w, k, v, alpha, beta, ha, initial_state = ctx.saved_tensors
        B, H, T, K, V = *r.shape, v.shape[-1]
        scale = ctx.scale
        ...
        dr = r.new_empty(NV, B, H, T, K)
        dw = w.new_empty(NV, B, H, T, K)
        ...
        fused_recurrent_bwd_kernel[grid](
            r, w, k, v, alpha, beta, ha, dht, dh0, do, dr, dk, dv, dalpha, dbeta, dha, initial_state,
            r.stride(1),
            v.stride(1),
            NK, scale,
            B=B, H=H, T=T, K=K, V=V,
        ...
        dalpha = dalpha.sum(0)
        dbeta = dbeta.sum(0)
        return dr.to(r), dw.to(w), dk.to(k), dv.to(v), dalpha.to(alpha), dbeta.to(beta), None, dh0, None


def fused_recurrent_dplr(
    r: torch.Tensor,
    w: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    ...
    r"""
    This function computes the recurrence S_t = S_t @ (W + alpha_t beta_t^T) + v_t k_t^T in a recurrent manner.
    Since the transition matrices is diagonal-plus-low-rank, we call it the Diagonal-Plus-Low-Rank (DPLR).

    Args:
        r (torch.Tensor) aka q:
            queries of shape `(B, H, T, K)`
        w (torch.Tensor) aka decay:
            queries of shape `(B, H, T, K)`
        k (torch.Tensor):
    ...
    o, final_state = FusedRecurrentFunction.apply(r, w, k, v, alpha, beta, scale, initial_state, output_final_state)
    return o, final_state
