...
def chunk_retention_fwd_kernel_o(
    ...
    for i_k in range(tl.cdiv(K, BK)):
        ...
        b_o += tl.dot((b_q * d_i[:, None]).to(b_q.dtype),
                      b_h, allow_tf32=False)
...
def chunk_retention_bwd_kernel_dqkv(
    ...
    o_i = tl.arange(0, BT)
    d_q, d_k = tl.math.exp2(
        (o_i + 1) * b_b), tl.math.exp2((BT - o_i - 1) * b_b)
...
class ChunkRetentionFunction(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, initial_state, output_final_state, scale, checkpoint_level):
        BT = 64
        h, final_state = chunk_fwd_h_fn(
            k, v, BT, initial_state, output_final_state)
        o = chunk_fwd_o_fn(h, q, k, v, BT, scale)
        if checkpoint_level == 1:
            h = None
        ctx.save_for_backward(q, k, v, h, initial_state, final_state)
    ...
    def backward(ctx, do, d_ht=None):
        ...
        q, k, v, h, initial_state, final_state = ctx.saved_tensors
        if h is None:
