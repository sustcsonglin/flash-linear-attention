...
def chunk_rwkv6_fwd_cumsum_kernel(
    ...
    p_o_minus_s = tl.make_block_ptr(o_minus_s + i_bh * T*S, (T, S), (S, 1), (i_t * BT, i_s * BS), (BT, BS), (1, 0))
    # [BT, BS]
...
def chunk_rwkv6_fwd_A_kernel_intra_sub_inter(
    ...
    for i_k in range(tl.cdiv(K, BK)):
        ...
        p_gn = tl.make_block_ptr(gi + i_bh * T*K, (T*K,), (1,), ((i_t * BT + i_j * BC + BC - 1) * K + i_k * BK,), (BK,), (0,))
        # [BK,]
...
def chunk_rwkv6_fwd_A_kernel_intra_sub_intra(
    ...
    for j in range(0, min(BC, T-i_t*BT-i_i*BC)):
        ...
        p_k = tl.make_block_ptr(k + i_bh * T*K, (T*K,), (1,), ((i_t * BT + i_j * BC + j) * K + i_k * BK,), (BK,), (0,))
        p_gk = tl.make_block_ptr(gi + i_bh * T*K, (T*K,), (1,), ((i_t * BT + i_j * BC + j) * K + i_k * BK,), (BK,), (0,))
        b_k = tl.load(p_k, boundary_check=(0,)).to(tl.float32)
        b_gk = tl.load(p_gk, boundary_check=(0,)).to(tl.float32)
        b_A += tl.sum(b_q * b_k[None, :] * tl.exp(b_g - b_gk[None, :]), 1)
        b_A = tl.where(o_i > j, b_A * scale, 0.)
        p_qi = tl.make_block_ptr(q + i_bh * T*K, (T*K,), (1,), ((i_t * BT + i_j * BC + j) * K + i_k * BK,), (BK,), (0,))
...
def chunk_rwkv6_fwd_kernel_inter(
    ...
    for i_k in range(tl.cdiv(K, BK)):
        ...
        p_ge = tl.make_block_ptr(g + i_bh * T*K, (T, K), (K, 1), (i_t * BT, i_k * BK), (BT, BK), (1, 0))
...
def chunk_rwkv6_bwd_kernel_intra(
    ...
    for j in range(0, min(BC, T-i_t*BT-i_i*BC)):
        ...
        p_gkj = tl.make_block_ptr(gi + i_bh * T*K, (T*K,), (1,), ((i_t * BT + i_i*BC+j) * K + i_k * BK,), (BK,), (0,))
        # [BC,]
    ...
    if i_i < max_block_idx - 1:
        p_gn = tl.make_block_ptr(gi + i_bh * T*K, (T*K,), (1,), ((i_t * BT + i_i * BC + BC - 1) * K + i_k * BK,), (BK,), (0,))
        # [BK,]
        b_gn = tl.load(p_gn, boundary_check=(0,))
    ...
    for j in range(0, min(BC, T-i_t*BT-i_i*BC)):
        ...
        p_gqj = tl.make_block_ptr(ge + i_bh * T*K, (T*K,), (1,), ((i_t * BT + i_i * BC + j) * K + i_k * BK,), (BK,), (0,))
        # [BC,]
...
def chunk_rwkv6_bwd_kernel_inter(
    ...
    p_gn = tl.make_block_ptr(gi + i_bh * T*K, (T*K,), (1,), (last_idx * K + i_k * BK,), (BK,), (0,))
    b_gn = tl.load(p_gn, boundary_check=(0,))
...
def chunk_rwkv6_fwd_intra_A_gated(q, k, gi, ge, u, scale, BT):
    BC = 16
    B, H, T, K = q.shape
    A = q.new_empty(B, H, T, BT, dtype=torch.float)
    ...
    # split then merge
        ...
        BK = 128
        NK = triton.cdiv(K, BK)
        A_intra = q.new_empty(NK, B, H, T, BC, dtype=torch.float)
        grid = (NK, NT * NC, B * H)
...
def chunk_rwkv6_bwd_dqk_intra(q, k, g_cumsum, g_cumsum_exclusive, dA, BT, scale):
    B, H, T, K = q.shape
    BC = 16
    BK = min(64, triton.next_power_of_2(K))
    NK = triton.cdiv(K, BK)
...
def chunk_rwkv6_bwd_dqkgu(q, k, v, h, g_cumsum, g_cumsum_exclusive, u, do, dh, dA, dq, dk, BT, scale):
    B, H, T, K, V = *q.shape, v.shape[-1]
    ...
    du = torch.empty(NT, B, H, K, dtype=torch.float, device=u.device)
...
def chunk_rwkv6_bwd_kernel_dh(
    ...
    for i_t in range(NT - 1, -1, -1):
        ...
        p_gk_last = gi + i_bg * T*K + last_idx * K + i_k * BK + tl.arange(0, BK)
...
def chunk_rwkv6_bwd_dh(q, k, v, g_cumsum, g_cumsum_exclusive, do, h0, dht, BT, scale, states_in_fp32=False):
    HQ = q.shape[1]
    ...
    if h0 is not None:
        dh0 = torch.empty_like(h0, dtype=torch.float) if h0.requires_grad else None
    else:
        dh0 = None
...
class ChunkRWKV6Function(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, g, u, scale, initial_state, output_final_state):
        BT = 64
        g_cumsum, g_cumsum_exclusive = chunk_rwkv6_fwd_cumsum(g, BT=BT)  # gi, ge for short
        h, ht = chunk_fwd_h(
            k=k,
            v=v,
            g=None,
        ...
        A = chunk_rwkv6_fwd_intra_A_gated(q, k, g_cumsum, g_cumsum_exclusive, u, scale, BT)
    ...
    def backward(ctx, do, dht):
        ...
        dv = chunk_gla_bwd_dv(k=k, g=g_cumsum, A=A, do=do, dh=dh, chunk_size=BT)
