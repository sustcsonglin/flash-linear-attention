...
def compute_final_dg(
    ...
    # b_o = tl.zeros([BT,], dtype=tl.float32)
    # for i_k in range(NK):
    #     p_dg = tl.make_block_ptr(dg + (i_bh + i_k * n_bh) * T, (T,), (1,), (i_t * BT,), (BT,), (0,))
    #     b_o += tl.load(p_dg, boundary_check=(0,))
...
def chunk_bwd_dqkg_fn(do, q, k, v, g, h, dh, scale):
    ...
    compute_final_dg[(NT, B*H)](dg, dg2, T=T, BT=BT, NK=NK)
    return dq, dk, dg2
...
def chunk_bwd_dv_kernel(
    ...
    b_A = b_A * tl.math.exp(b_g[None, :] - b_g[:, None]) * scale
...
class SimpleGLAFunction(torch.autograd.Function):
    ...
    def backward(ctx, do, dht):
        ...
        h, final_state = chunk_fwd_h_fn(k=k, v=v, g=g, gk=None, gv=None, BT=BT, h0=initial_state, output_final_state=False, states_in_fp32=False)
        dh, dh0 = chunk_bwd_dh_fn(q=q, k=k, v=v, g=g, gk=None, gv=None, do=do, h0=initial_state, dht=dht, BT=BT, scale=scale, states_in_fp32=False)
