...
def chunk_retention_fwd_kernel(
    ...
    # [BD, DV]
    b_h = tl.zeros([BD, DV], dtype=tl.float32)
...
def chunk_retention_bwd_kernel(
    ...
    for i in range(0, tl.cdiv(T, BT)):
        p_k = tl.make_block_ptr(k + i_bh * s_qh, (T, BD), (s_qt, s_qd), (i * BT, 0), (BT, BD), (1, 0))
        ...
        p_dq = tl.make_block_ptr(dq + i_bh * s_qh, (T, BD), (s_qt, s_qd), (i * BT, 0), (BT, BD), (1, 0))
    ...
    d_s = tl.trans(d_s)
    # [BD, BD]
    b_dh = tl.zeros([BD, BD], dtype=tl.float32)
    for i in range(1, tl.cdiv(T, BT) + 1):
        ...
        p_do = tl.make_block_ptr(do + i_bh * s_qh, (T, BD), (s_qt, s_qd), (T - i * BT, 0), (BT, BD), (1, 0))
        p_dk = tl.make_block_ptr(dk + i_bh * s_qh, (T, BD), (s_qt, s_qd), (T - i * BT, 0), (BT, BD), (1, 0))
        p_dv = tl.make_block_ptr(dv + i_bh * s_qh, (T, BD), (s_qt, s_qd), (T - i * BT, 0), (BT, BD), (1, 0))
...
class ChunkRetentionFunction(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v):
        ...
        grid = (triton.cdiv(BD, DV), batch_size * n_heads)
